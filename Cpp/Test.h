...
namespace Algorithm {
    ...
    public:
      ...
      template <class T>
      static bool isVectorSorted(const vector<T>& ifSorted, bool needPrint = true, function<bool(const T &, const T &)> comparator = std::less<T>()){
        using SizeT = decltype(ifSorted.size());
        for(SizeT i = 0; i + 1<ifSorted.size();){
          if(comparator(ifSorted[i],ifSorted[i+1]) ||
              (!comparator(ifSorted[i],ifSorted[i+1])&&!comparator(ifSorted[i+1],ifSorted[i]))){
            ++i;
          } else{
